"""
OAS Enhancer — FastAPI router.

Drop this into any FastAPI app:

    from backend.router import router as oas_router
    from backend.pipeline import init_client
...
from .pipeline import get_suggestions

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/oas-enhancer", tags=["OAS Enhancer"])
...
def _get_yaml_context(spec: dict, suggestion: dict) -> dict:
    ...
    try:
        if method == "component":
            ...
            section     = parts_loc[0]
            schema_name = parts_loc[1] if len(parts_loc) > 1 else None
    ...
    final_key     = parts[-1]
    inserted_line = f"{' ' * indent}{final_key}: {json.dumps(value)}"
    ...
    for s in suggestions:
        ...
        if missing or not s.get("location") or not s.get("method"):
            logger.warning(
                ...
                missing or "location/method empty",
                {k: s.get(k) for k in ("method", "path", "location", "field")},
            )
            dropped += 1
            continue
        try:
...
async def suggest(
    ...
    async def event_stream():
        try:
            async for event in get_suggestions(oas_spec, postman_text, has_postman, force_refresh == "true"):
                if event.get("type") == "done":
                    event["suggestions"] = _enrich_suggestions(event["spec"], event["suggestions"])
                    if not event["suggestions"]:
                        yield f"data: {_dumps({'type': 'error', 'message': 'No applicable suggestions found. The spec may already be complete, or all suggestions were invalid. Check server logs for details.'})}\n\n"
                        return
