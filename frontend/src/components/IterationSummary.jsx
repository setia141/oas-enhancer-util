...
function Badge({ count, color }) {
  ...
  return (
    <span style={{
      ...
      background: color + '22', color, fontWeight: 600, marginLeft: 8,
    }}>
      {count}
    </span>
  )
...
function IterationCard({ data, defaultOpen }) {
...
export default function IterationSummary({ iterations, summary }) {
  if (!iterations?.length) return (
    <div style={{ padding: 24, color: '#888', textAlign: 'center' }}>No iteration data available.</div>
  )
